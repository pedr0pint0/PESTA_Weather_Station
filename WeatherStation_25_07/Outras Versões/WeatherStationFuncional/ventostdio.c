...
int main() {
    // Passo 1: Inicializar o sensor (tentar até conseguir)
    int sensor_ok = 0;
    while (!sensor_ok) {
   ...
   // printf("Sensor inicializado com sucesso!\n");
    ...
    // Passo 2: Ciclo infinito para escutar pedidos
    char buffer[32];  // onde vamos guardar o comando lido

    while (1) {
       // printf("À espera de um comando (ex: LER)...\n");

        // Passo 3: Lê o comando do stdin
        if (fgets(buffer, sizeof(buffer), stdin) != NULL) {
            // Remover o \n no fim do input
            buffer[strcspn(buffer, "\r\n")] = '\0';

            // Passo 4: Verificar se o comando é "LER"
            if (strcmp(buffer, "LER") == 0) {
                // Passo 5: Ler valores do sensor
                float velocidade = readWindSpeed();
                int direcao = readWindDirection();

                // Passo 6: Validar os valores
                if (velocidade >= 0 && direcao >= 0 && direcao <= 360) {
                    // Passo 7: Escrever os dados no stdout
                    printf("%.2f %d\n", velocidade, direcao);
            ...
            // fgets devolveu NULL (erro ou fim de stdin)
           // fprintf(stderr, "Erro ao ler do stdin ou stdin fechado.\n");
            break;  // sai do while
        }

        // Esperar um pouco antes do próximo ciclo
